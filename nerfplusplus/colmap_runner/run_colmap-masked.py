...
def prepare_mvs(img_dir, sparse_dir, mvs_dir):
    ...
    cmd = ' image_undistorter \
            ...
            --output_path {} \
            --output_type COLMAP \
            --max_image_size 2000'.format(img_dir, os.path.join(sparse_dir, '0'), mvs_dir)
                                                ##Testat att ändra ovan till os.path.join(sparse_dir, '0'). Blir fel pga mvs fel pga sparse/0 inte existerar
    bash_run(cmd)
